...
public class NegociosService {
    ...
    public void introducirVenta(int ncliente, int nproducto, String vend) {
        try {
            ...
            for (int i = 0; i < clientes.size() && clienteVenta == null; i++) {
                if (/*clientes.get(i).getIdCliente() == ncliente &&*/buscarCliente(ncliente) != null) {
                    clienteVenta = clientes.get(i);
                }
            }
            if (clienteVenta == null) {
                Exception e = new RuntimeException("El cliente no existe.");
            ...
            for (int i = 0; i < productos.size() && productoVenta == null; i++) {
                if (/*productos.get(i).getId() == nproducto &&*/buscarProducto(nproducto) != null) {
                    productoVenta = productos.get(i);
                }
            }
            if (productoVenta == null) {
                Exception e = new RuntimeException("El producto no existe.");
                System.out.println(e.getMessage());
                throw e;
            }
    ...
    public void buscarVenta(int nv) {
        try {
            ...
            if (venta == null) {
                Exception e = new Exception("La venta con id: " + nv + " no existe");
                System.out.println(e.getMessage());
                throw e;
            }
